...
do
	...
	function Matchmake:drawRanked(viewElement)
		...
		if (TB_MENU_PLAYER_INFO.ranking.elo) then
			...
			local winrate = math.floor(TB_MENU_PLAYER_INFO.ranking.wins / games * 100 + 0.5) / 100
			mmRankedInfoGames:addCustomDisplay(true, function()
					mmRankedInfoGames:uiText(games .. " fights total, " .. winrate .. "% win rate", nil, nil, 4, LEFT, 0.7)
				end)
		end
		...
		rankedSearchButton:addCustomDisplay(false, function()
				rankedSearchButton:uiText("Search", nil, rankedSearchButton.pos.y + 22, FONTS.BIG, nil, 0.7, nil, 1)
			end)
		...
		if (TB_MENU_PLAYER_INFO.data.qi < 500) then
			local rankedUnavailable = UIElement:new({
				parent = viewElement,
				pos = { 0, 0 },
	...
	function Matchmake:drawUnranked(viewElement)
		...
		local unrankedSearchProgress = UIElement:new({
			parent = viewElement,
			pos = { viewElement.size.w / 6 + 30, -125 },
			size = { viewElement.size.w / 3 * 2, 20 }
		})
		local progress, rotation = 0, 0
		unrankedSearchProgress:addCustomDisplay(false, function()
				unrankedSearchProgress:uiText(searchStr.part1 .. searchStr.str .. searchStr.part2, nil, nil, nil, LEFT)
				set_color(1,1,1,0.8)
		...
		unrankedSearchStop:addCustomDisplay(false, function()
				unrankedSearchStop:uiText("Stop Search", nil, unrankedSearchStop.pos.y + 8, FONTS.BIG, nil, 0.7, nil, 1)		
			end)
		...
		unrankedBestMatch:addCustomDisplay(false, function()
				unrankedBestMatch:uiText("Find", nil, unrankedBestMatch.pos.y + 10, FONTS.BIG, nil, 0.7, nil, 1)
				unrankedBestMatch:uiText("best match", nil, unrankedBestMatch.pos.y + 55, nil, nil, 0.9, nil, 0.6)
			end)
		unrankedBestMatch:addMouseHandlers(nil, function()
				...
				if (not unrankedSearchProgress:uiText(searchStr.part1 .. searchStr.str .. searchStr.part2, nil, nil, nil, LEFT, nil, nil, nil, nil, nil, nil, true)) then
					unrankedSearchProgress.shift.y = -140
		...
		unrankedSearchSettings:addCustomDisplay(false, function()
				unrankedSearchSettings:uiText("Search", nil, unrankedSearchSettings.pos.y + 10, FONTS.BIG, nil, 0.7, nil, 1)
				unrankedSearchSettings:uiText("with settings", nil, unrankedSearchSettings.pos.y + 55, nil, nil, 0.9, nil, 0.6)
			end)
