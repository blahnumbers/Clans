...
add_trail_particle(0, 1, 0, 0, 0, 0, 0, 0)
do
	PlayerInfo = {}
	...
	function PlayerInfo:getColors(file)
		...
		for ln in file:lines() do
			if string.match(ln, "^FORCOL 0;0") then
				...
				colors.force = tonumber(ln:match("%d+%S"))
			end
			if string.match(ln, "^RELCOL 0;0") then
				ln = ln:gsub("^RELCOL 0;0", "")
				colors.relax = tonumber(ln:match("%d+%S"))
			end
		end
		return colors
	end
	...
	function PlayerInfo:getClan(player)
		...
		for ln in file:lines() do
			if string.match(ln, "^CLAN 0;") then
				...
				clanInfo.clanid = tonumber(clanid)
				if (clanInfo.clanid ~= 0) then
					clanInfo.clanTag = ln:match("%S+$")
				end
				file:close()
				break
			end
		...
		for ln in file:lines() do
			if string.match(ln, "^CLAN") then
				...
				if (tonumber(data_stream[2]) == clanInfo.clanid) then
					data_stream[3] = data_stream[3]:gsub('%W+%d+%W*', '')
					clanInfo.clanName = data_stream[3]
					break
				end
			end
		end
	...
	function PlayerInfo:getBeltFromQi(qi)
		...
		if (qi < 20) then 
			belt.name = "White"
			belt.icon = "/system/belts/white.tga"
		elseif (qi < 50) then
			belt.name = "Yellow"
			belt.icon = "/system/belts/yellow.tga"
		elseif (qi < 100) then
			belt.name = "Orange"
			belt.icon = "/system/belts/orange.tga"
		elseif (qi < 200) then
			belt.name = "Green"
			belt.icon = "/system/belts/green.tga"
		elseif (qi < 500) then
			belt.name = "Blue"
			belt.icon = "/system/belts/blue.tga"
		elseif (qi < 1000) then
			belt.name = "Brown"
			belt.icon = "/system/belts/brown.tga"
		elseif (qi < 2000) then
			belt.name = "Black"
			belt.icon = "/system/belts/black.tga"
		elseif (qi < 3000) then
			belt.name = "2nd Dan Black"
			belt.icon = "/system/belts/black2dan.tga"
		elseif (qi < 4000) then
			belt.name = "3rd Dan Black"
			belt.icon = "/system/belts/black3dan.tga"
		elseif (qi < 5000) then
			belt.name = "4th Dan Black"
			belt.icon = "/system/belts/black4dan.tga"
		elseif (qi < 6000) then
			belt.name = "5th Dan Black"
			belt.icon = "/system/belts/black5dan.tga"
		elseif (qi < 7000) then
			belt.name = "6th Dan Black"
			belt.icon = "/system/belts/black6dan.tga"
		elseif (qi < 8000) then
			belt.name = "7th Dan Black"
			belt.icon = "/system/belts/black7dan.tga"
		elseif (qi < 9000) then
			belt.name = "8th Dan Black"
			belt.icon = "/system/belts/black8dan.tga"
		elseif (qi < 10000) then
			belt.name = "9th Dan Black"
			belt.icon = "/system/belts/black9dan.tga"
		elseif (qi < 15000) then
			belt.name = "10th Dan Black"
			belt.icon = "/system/belts/black10dan.tga"
		elseif (qi < 20000) then
			belt.name = "Master"
			belt.icon = "/system/belts/master.tga"
		elseif (qi < 50000) then
			belt.name = "God"
			belt.icon = "/system/belts/god.tga"
		elseif (qi < 100000) then
			belt.name = "One"
			belt.icon = "/system/belts/one.tga"
		else
			belt.name = "Elite"
			belt.icon = "/system/belts/elite.tga"
		end
		return belt
	end
